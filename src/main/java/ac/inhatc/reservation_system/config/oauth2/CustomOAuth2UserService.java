...
public class CustomOAuth2UserService extends DefaultOAuth2UserService {
    ...
    private Member updateExistingMember(Member member, OAuth2UserInfo userInfo, String accessToken) {
        ...
        if (userInfo.getProfileImage() != null && !userInfo.getProfileImage().equals(member.getProfileImage())) {
            member.setProfileImage(userInfo.getProfileImage());
            needUpdate = true;
        }
        // 이메일이 변경된 경우 (네이버 연락처 이메일 변경 등)
        if (userInfo.getEmail() != null && !userInfo.getEmail().equals(member.getEmail())) {
            // 변경하려는 이메일이 이미 다른 회원이 사용 중인지 확인
            Optional<Member> existingEmail = memberRepository.findByEmail(userInfo.getEmail());
            if (existingEmail.isEmpty()) {
                member.setEmail(userInfo.getEmail());
                needUpdate = true;
                log.info("📧 OAuth 회원 이메일 업데이트: {} → {}", member.getEmail(), userInfo.getEmail());
            }
        }

        // Access Token 항상 업데이트 (로그인할 때마다 새 토큰 발급됨)
    ...
    private Member createNewMember(AuthProvider provider, OAuth2UserInfo userInfo, String accessToken) {
        ...
        // 이메일이 없거나 이미 사용 중인 경우 고유한 이메일 생성
        ...
        Member member = Member.builder()
                ...
                .oauthAccessToken(accessToken)  // Access Token 저장
                .role(Role.USER)  // 기본 역할: 일반 사용자
                .build();
